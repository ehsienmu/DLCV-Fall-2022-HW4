...
def config_parser():
    ...
    parser.add_argument("--render_test_dlcv", action='store_true')
    parser.add_argument("--render_test_dlcv_json_path", type=str, default='')
    parser.add_argument("--render_test_dlcv_output_path", type=str, default='')
    parser.add_argument("--render_test_dlcv_model_file", type=str, default='')
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, render_filenames=None):
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            # # pdb.set_trace()
            if (render_filenames is not None):
                filename = os.path.join(savedir, render_filenames[i].split('/')[-1].split('.')[0] + '.png')
                # # pdb.set_trace()
            else:
...
def load_everything_dlcv(args, cfg):
    ...
    kept_keys = {
            ...
            'poses', 'render_poses', 'images', 'fnames'}
...
if __name__=='__main__':
    ...
    if args.render_test_dlcv:
        cfg['data']['render_test_dlcv_json_path'] = args.render_test_dlcv_json_path
        # cfg['render_test_dlcv_output_path'] = args.render_test_dlcv_output_path
    ...
    # pdb.set_trace()
    if args.render_test_dlcv:
        print('load_everything_dlcv')
        data_dict = load_everything_dlcv(args=args, cfg=cfg)
    else:
    ...
    if args.render_test_dlcv:
        ...
        testsavedir = args.render_test_dlcv_output_path
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        # pdb.set_trace()
        rgbs, depths, bgmaps = render_viewpoints(
                render_poses=data_dict['poses'][data_dict['i_test']],
                HW=data_dict['HW'][data_dict['i_test']][0],
                Ks=data_dict['Ks'][data_dict['i_test']][0],
                gt_imgs=None,
                savedir=testsavedir, dump_images=args.dump_images,
                eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg,
                render_filenames=data_dict['fnames'],
                **render_viewpoints_kwargs)
        # imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        # imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30, quality=8)
    # # render testset and eval
