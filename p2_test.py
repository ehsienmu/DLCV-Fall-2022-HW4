...
if __name__ == '__main__':
    ...
    with open('./office_translate.json', 'r') as f:
        ...
        inv_office_translate_dict = {v: k for k, v in office_translate_dict.items()}
    # fixed random seed
    fixed_seed(1)
    ...
    val_set = CustomImageDataset(data_folder_path=args.input_dir, csv_file_path=args.input_csv_file, translate_dict=office_translate_dict, have_label=False, transform=tfm)
    ...
    with torch.no_grad():
        ...
        for _, (image_id, data, fname,) in enumerate(tqdm(val_loader)):
            ...
            # pdb.set_trace()
            results.append(((str(int(image_id[0].detach().cpu()))), fname[0], str(int(pred[0].detach().cpu()))))
    ...
    with open(args.output_file, 'w') as f:
        ...
        for iid, fname, predl in results:
            # pdb.set_trace()
            f.write(iid)
            f.write(',')
            f.write(fname)
            f.write(',')
            f.write(inv_office_translate_dict[int(predl)])
            f.write('\n')
